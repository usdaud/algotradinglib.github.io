...
Where:
- \(N\) is the total number of assets in the market.
- \(V_j\) is the total market capitalization of asset \(j\).
...
- \(\beta_m\) is the beta of the market, which is 1.
...
Where:
- \(R_p\) and \(\sigma_p\) are the expected return and standard deviation of the portfolio \(p\).

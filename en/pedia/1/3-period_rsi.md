...
1. **Determine the Up and Down Days**:
   - Calculate the daily price changes for the asset over the past 3 periods.
...
2. **Calculate the Average Gain and Average Loss**:
   - The average gain over the last 3 periods is the sum of all up day changes divided by 3.
...
3. **Compute the Relative Strength (RS)**:
   - RS is calculated as the ratio of the average gain to the average loss over the 3 periods.

4. **Calculate the RSI**:
   - Plug the RS value into the RSI formula to compute the 3-period RSI.

### Example Calculation

Suppose we have the following closing prices for an asset over four days: $50, $52, $51, and $53.
...
3. **Swing Rejections**:
   ...
   - Bearish Rejection: RSI rises above 70 (overbought), falls below 70, rises again but stays below 70, and then falls below its previous low.
...
1. Calculate the 3-period RSI for the asset.
...
RSI can also be used in trend-following strategies, providing a confirmation signal for trend direction.

#### Simple Trend Following Algorithm

1. Calculate a longer-term moving average (e.g., 50-period) to determine the trend direction.
2. Calculate the 3-period RSI for the asset.
3. Enter a long position if the moving average indicates an uptrend and the RSI rises above 50 from below.
4. Enter a short position if the moving average indicates a downtrend and the RSI falls below 50 from above.
5. Exit positions based on opposite signals or predefined profit/loss targets.
...
1. Calculate the 3-period RSI and Moving Average Convergence Divergence (MACD).
2. Enter a long position when:
   - The 3-period RSI is below 30, indicating oversold conditions.
   - The MACD line crosses above the signal line, indicating bullish momentum.
3. Enter a short position when:
   - The 3-period RSI is above 70, indicating overbought conditions.
   - The MACD line crosses below the signal line, indicating bearish momentum.
4. Exit positions based on opposite signals or predefined profit/loss targets.

...
At the most fundamental level, the value of an option is composed of its intrinsic value and extrinsic value (also called time value). The intrinsic value reflects the amount by which an option is in-the-money, whereas the extrinsic value encompasses the premium paid for the potential of the option to gain value before expiration. Time decay directly impacts the extrinsic value of an option. As expiration nears, the likelihood of significant price movement diminishes, leading to a decrease in the option’s extrinsic value.

In mathematical terms, theta (Θ) is one of the "Greeks" that options traders use to assess risk and potential returns. Specifically, theta measures the sensitivity of an option's price to the passage of time, and it is typically expressed as a negative number, indicating the loss of value per day. For example, if an option has a theta of -0.05, it will lose $5 for each day that passes, provided all other conditions remain constant.
...
A calendar spread involves buying and selling options with the same strike price but different expiration dates. This strategy capitalizes on the differential time decay rates of short-term and long-term options. Typically, a trader buys a longer-term option and sells a shorter-term option. As the shorter-term option decays faster, the net effect can be a gain for the trader. Calendar spreads can be employed to take advantage of expected movements in volatility and to hedge against adverse effects of time decay.

#### 2. Diagonal Spreads

Diagonal spreads are similar to calendar spreads but involve options with different strike prices and expiration dates. This strategy offers flexibility in adjusting the risk-reward profile of a portfolio. By choosing appropriate strike prices and expiration dates, traders can create a position that benefits from time decay while retaining potential for gains if the underlying asset moves favorably.

#### 3. Covered Calls

A covered call strategy involves owning the underlying asset and selling call options on it. This strategy generates income from the premiums received from selling call options, which can offset the effects of time decay on the long options position. Additionally, covered calls provide a buffer against modest declines in the underlying asset's price.
...
Rolling options involves closing an existing options position and simultaneously opening a new position with a later expiration date. This strategy helps manage time decay by extending the time horizon of the options position. Rolling options can be used to maintain exposure to the underlying asset while mitigating the impact of time decay.
...
The rate of time decay accelerates as options approach expiration. Longer-term options have slower time decay compared to near-term options. Traders need to assess the time horizon of their positions and choose strategies that align with their time decay expectations.

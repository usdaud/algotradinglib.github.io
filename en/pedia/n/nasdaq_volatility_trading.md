...
- **[Historical Volatility](../h/historical_volatility.md) (HV)**: The standard deviation of returns over a specified period, providing an empirical measure based on past price movements.
- **Implied Volatility (IV)**: Derived from the price of options, it reflects the market's expectation of future volatility.
- **Beta**: Measures the sensitivity of a stock's return relative to the overall market, often compared against a broad index like the S&P 500 or NASDAQ-100.
...
- **Straddles and Strangles**: Purchasing both call and [put options](../p/put_options.md) to profit from a significant price movement in either direction.
- **Butterfly Spreads**: A strategy to capture profit from low volatility and slight price movements.
- **Vega-neutral Strategies**: Trades designed to isolate movements in implied volatility separately from price movements.

### Futures and ETFs

- **Volatility Futures**: These are [futures contracts](../f/futures_contracts.md) on volatility indices like the CBOE Volatility Index (VIX). Though the VIX tracks volatility for the S&P 500, there are ETFs and ETPs pegged to this index that traders might use to gain exposure to NASDAQ volatility.
- **Volatility ETFs/ETNs**: Exchange-Traded Funds (ETFs) and Exchange-Traded Notes (ETNs) like ProShares' VIXY and UVXY provide direct access to trade volatility-based products.

## Strategies in NASDAQ Volatility Trading

### Mean Reversion

Since volatility is often mean-reverting, traders employ [mean reversion](../m/mean_reversion.md) strategies to profit from the tendency of volatility to revert back to an average level after periods of high or low volatility. Techniques such as statistical [arbitrage](../a/arbitrage.md) and applying [Bollinger Bands](../b/bollinger_bands.md) can be useful.
...
Algo trading and HFT involve the use of sophisticated algorithms to execute trades at absurdly high speeds, exploiting price inefficiencies and liquidity in NASDAQ markets. Companies like Jane Street, Citadel Securities, and Renaissance Technologies use complex [quantitative models](../q/quantitative_models.md) to trade on volatility. Jane Street: [Jane Street](https://janestreet.com/) | Citadel Securities: [Citadel Securities](https://www.citadelsecurities.com/) | Renaissance Technologies: [Renaissance Technologies](https://www.rentec.com/)

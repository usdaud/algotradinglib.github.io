...
Before delving into underapplied overhead, it is essential to understand the predetermined [overhead rate](../o/overhead_rate.md). This rate is calculated at the beginning of an [accounting](../a/accounting.md) period based on budgeted overhead costs and an expected level of activity (e.g., machine hours, labor hours). The formula is as follows:

```
Predetermined [Overhead Rate](../o/overhead_rate.md) = Budgeted Overhead Costs / Expected Activity Level
```

### Applied Overhead

Applied overhead refers to the amount of overhead allocated to specific jobs or production units based on the predetermined [overhead rate](../o/overhead_rate.md). It is computed as:

```
Applied Overhead = Predetermined [Overhead Rate](../o/overhead_rate.md) × Actual Activity Level
```
...
1. **Determine Actual Overhead Costs:** Collect all overhead expenses incurred during the [accounting](../a/accounting.md) period.
2. **Calculate Applied Overhead:** Use the predetermined [overhead rate](../o/overhead_rate.md) to allocate overhead to products or jobs based on actual activity levels.
...
Underapplied overhead has several implications for a [business](../b/business.md):

1. **[Financial Statements](../f/financial_statements.md):** The underapplied overhead must be adjusted in the company’s [financial statements](../f/financial_statements.md), often by increasing the cost of goods sold (COGS) to reflect the additional overhead costs not initially applied.
...
3. **Cost Increases**: Unexpected increases in overhead costs, such as [utility](../u/utility.md) rate hikes or higher than anticipated [maintenance expenses](../m/maintenance_expenses.md), can cause underapplication.
...
2. **[Variance Analysis](../v/variance_analysis.md)**: Conduct regular [variance analysis](../v/variance_analysis.md) to identify and address discrepancies between estimated and actual overhead costs.
3. **Improved Cost Tracking**: Enhance the tracking of overhead costs by implementing [robust](../r/robust.md) [internal controls](../i/internal_controls.md) and [accounting](../a/accounting.md) systems.

### Illustrative Example

Imagine a [manufacturing](../m/manufacturing.md) company, ABC Corp., with the following data for a given period:
...
The company would then need to adjust its [financial statements](../f/financial_statements.md) to account for the $10,000 underapplied overhead.

### Conclusion

Understanding and managing underapplied overhead is critical for businesses aiming for accurate cost allocation, financial reporting, and overall [efficiency](../e/efficiency.md). By regularly reviewing overhead costs, updating predetermined rates, and conducting variance analyses, companies can better control their cost structures and maintain financial accuracy. Proper [accounting](../a/accounting.md) treatment of underapplied overhead ensures [transparency](../t/transparency.md) in [financial statements](../f/financial_statements.md), thereby supporting informed decision-making and strategic planning.
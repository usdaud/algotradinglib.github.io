...
1. **Utility Function**: Represents an investor's preferences, translating different levels of wealth or returns into a quantitative measure of satisfaction. Utility functions are often defined to capture the investor’s attitude towards risk, with common forms including:
   ...
   - **Quadratic Utility Function**: Often used in [mean-variance optimization](../m/mean-variance_optimization.md).
...
- **Optimization Algorithms**: Employing algorithms such as [mean-variance optimization](../m/mean-variance_optimization.md) or [stochastic optimization](../s/stochastic_optimization.md) to select asset weights that maximize expected utility.

#### Asset Allocation
Utility Theory informs strategic [asset allocation](../a/asset_allocation.md) by balancing the trade-offs between expected return and risk. Techniques include:
- **Mean-[Variance Analysis](../v/variance_analysis.md)**: A method developed by Harry Markowitz that incorporates utility functions to propose an optimal mix of assets.
- **Dynamic [Asset Allocation](../a/asset_allocation.md)**: Adjusting portfolio weights over time in response to changes in market conditions and investor’s circumstances to maintain or improve utility.
...
- **Risk-Adjusted Returns**: Evaluating returns adjusted for risk using utility-based metrics like the [Sharpe Ratio](../s/sharpe_ratio.md), which considers the trade-off between return and volatility.

### Utility Theory Models
Several models in [portfolio management](../p/portfolio_management.md) are underpinned by Utility Theory:
- **Capital Asset Pricing Model (CAPM)**: Integrates [utility maximization](../u/utility_maximization.md) by assuming investors are risk-averse and markets are in equilibrium. It describes the relationship between [systematic risk](../s/systematic_risk.md) and expected return on assets.
...
Financial institutions employ Utility Theory extensively in client advisory services and [portfolio management](../p/portfolio_management.md). They customize portfolio recommendations based on the risk preferences and utility functions specific to each investor:
- **Wealthfront**: An example of a robo-advisor platform that uses algorithms based on Utility Theory to offer personalized investment advice and [portfolio management](../p/portfolio_management.md). Learn more at [Wealthfront](https://www.wealthfront.com/).

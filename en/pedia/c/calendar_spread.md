...
A calendar spread, also called a time spread, involves buying and selling options of the same strike but different expirations. It is designed to benefit from time decay differences and changes in implied volatility.

## Structure
- Buy a longer-dated option
- Sell a shorter-dated option at the same strike

The position can be constructed with calls or puts.

## Payoff Profile
The strategy typically profits if the underlying stays near the strike at the short option expiration. Time decay on the short option works in favor of the spread.

## Use Cases
Calendar spreads are used when a trader expects low short-term volatility and higher longer-term volatility. They are also used to position for events that occur after the short option expires.

## Risks
The strategy is sensitive to implied volatility and large price moves. A sharp move away from the strike can reduce the value of the long option and create losses.

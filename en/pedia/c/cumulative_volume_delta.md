...
Cumulative volume delta measures the difference between buying and selling volume over time. It is an order flow tool that tracks aggressive buying versus aggressive selling.

## Calculation
Volume delta is typically estimated by classifying trades at the ask as buying and trades at the bid as selling. The cumulative sum of these values forms the indicator.

## Interpretation
- Rising delta suggests sustained buying pressure
- Falling delta suggests sustained selling pressure
- Divergence between delta and price can signal hidden weakness or strength

## Use in Trading
Traders use cumulative volume delta to confirm breakouts, identify absorption, and evaluate the strength of trends.

## Limitations
The indicator depends on trade classification methods, which can vary by data provider. It can also be noisy in low liquidity environments.

## Inputs and Parameters

Most implementations of Cumulative Volume Delta use a lookback window and one or more smoothing steps. The lookback controls how quickly the indicator reacts; shorter values respond faster but produce more noise. Smoothing can be a simple moving average or another filter, and it should be consistent with the timeframe of the strategy.

When applying Cumulative Volume Delta across different instruments, normalize inputs so the indicator remains comparable. For price based indicators, using median or typical price can reduce distortion from single prints. Volume based indicators require clean volume data and consistent session definitions.
...
Cumulative Volume Delta tends to behave differently in trending and ranging markets. In strong trends it can stay above or below its baseline for extended periods, which makes mean reversion signals risky. In ranges it can oscillate frequently and generate whipsaws.

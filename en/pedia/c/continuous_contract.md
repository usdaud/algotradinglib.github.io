...
A continuous contract is a synthetic price series that stitches together multiple futures contracts to create a longer historical dataset.

## Methods
- Front month roll: switch to the next contract at a predefined roll date
- Back-adjusted: adjust prior prices to remove gaps at roll
- Forward-adjusted: shift the current contract to align with historical prices

## Use in Trading
Continuous contracts are used for backtesting, research, and long-term analysis. They provide a consistent time series but depend on the chosen roll methodology.

## Limitations
Roll methods can distort price levels and returns. Researchers should document roll rules and avoid mixing series types without adjustment.

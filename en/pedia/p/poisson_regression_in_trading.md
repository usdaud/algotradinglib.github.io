...
where \(\lambda\) is the rate (mean number of events in the interval), \(e\) is the base of natural logarithm, \(y\) is the actual number of events.

The mean \(\lambda\) is typically modeled as an exponential function of the independent variables to ensure that \(\lambda\) is always positive. Therefore, in Poisson regression,

\[ \lambda = e^{\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_k X_k} \]

or

\[ \log(\lambda) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_k X_k \]

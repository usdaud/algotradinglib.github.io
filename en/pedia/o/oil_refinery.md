# Oil Refinery

An oil refinery is an industrial process plant where [crude oil](../c/crude_oil.md) is processed and refined into more useful products such as gasoline, diesel fuel, kerosene, jet fuel, and various other [petroleum](../p/petroleum.md)-based products. Refineries are critical components in the [supply chain](../s/supply_chain.md) of the oil & gas [industry](../i/industry.md) because they transform raw [crude oil](../c/crude_oil.md) into consumer-ready products that fuel our daily lives.

## Overview of the Refining Process

The refining process generally involves two primary stages: separation and conversion.

### Separation

**Separation** is the first stage of refining and consists of distillation and other separation techniques to split the [crude oil](../c/crude_oil.md) into various components, or fractions, based on their boiling points.

#### Distillation

1. **[Crude Oil](../c/crude_oil.md) Distillation**: 
   - [Crude oil](../c/crude_oil.md) is first heated in a furnace.
   - It is then fed into a distillation column, also called a fractionating column.
   - As the oil heats up, different components vaporize and rise through the column at different levels.
   - Lighter fractions such as gasoline rise to the top, while heavier fractions settle at the bottom.

#### Additional Separation Techniques

- **Vacuum Distillation**: Used to separate heavier fractions of [crude oil](../c/crude_oil.md) that require lower pressure to vaporize.
- **Deasphalting**: Removes heavy asphalt-like substances from the remaining oil fractions.

### Conversion

**Conversion** processes further refine these separated fractions to improve their properties and increase the [yield](../y/yield.md) of lighter, more valuable products. Some key conversion processes include:

#### Cracking

- **Fluid Catalytic Cracking (FCC)**: Uses a catalyst to crack larger molecules into smaller, more valuable products such as gasoline and olefins.
- **Hydrocracking**: Similar to FCC but uses hydrogen to crack heavier molecules and improve product quality.

#### Reforming

- **Catalytic Reforming**: Converts low-octane hydrocarbons into high-octane products for gasoline blending by rearranging molecular structures.

#### Alkylation and Polymerization

- **Alkylation**: Joins smaller [hydrocarbon](../h/hydrocarbon.md) molecules to form higher-octane products for gasoline.
- **Polymerization**: Similar to alkylation but usually employs different catalysts and conditions.

### Treatment and Blending

Post-separation and -conversion, various fractions undergo treatment to remove impurities such as sulfur and nitrogen compounds. Examples include:

- **Desulfurization**: Uses hydrogen to remove sulfur impurities from fuel products.
- **Amine Treating**: Uses chemical amines to remove acidic gases.

Finally, different fractions are blended to produce final [petroleum](../p/petroleum.md) products with desired specifications.

## Major Units in an Oil Refinery

### Crude Distillation Unit (CDU)

The CDU is where the preliminary separation of [crude oil](../c/crude_oil.md) into different fractions occurs.

### Vacuum Distillation Unit (VDU)

Processes the heavier fractions recovered from the CDU.

### Fluid Catalytic Cracking Unit (FCCU)

Cracks large [hydrocarbon](../h/hydrocarbon.md) molecules into smaller, more valuable products.

### Hydrocracker Unit

Performs a similar role to the FCC but uses hydrogen in addition to catalysts for cracking.

### Reforming Unit

Upgrades naphtha into high-octane reformate, which is used in gasoline blending.

### Alkylation Unit

Combines light [hydrocarbon](../h/hydrocarbon.md) gases into a high-octane gasoline component.

### Coker Unit

Processes residuum from the crude distillation process to create lighter hydrocarbons and [petroleum](../p/petroleum.md) coke.

## Economic Aspects

### Costs and Margins

The economic viability of an oil refinery primarily depends on the cost of [crude oil](../c/crude_oil.md) and the [margin](../m/margin.md) derived from refining activities.

- **Refinery [Margin](../m/margin.md)**: The difference between the cost of [crude oil](../c/crude_oil.md) and the sales price of the refined products.
- **Operating Costs**: Include energy costs, labor, maintenance, and other operational expenditures.

### Market Dynamics

The price of refined products such as gasoline and diesel is influenced by global [demand](../d/demand.md) and [supply](../s/supply.md) dynamics, regulatory policies, and technological advancements.

### Environmental and Regulatory Aspects

Refineries face stringent environmental regulations aimed at reducing emissions and managing waste. Compliance with these regulations often incurs additional costs but also drives innovation in cleaner refining technologies.

## Technological Innovations

### Advanced Process Control (APC)

APC systems optimize refinery operations by employing algorithms to control process variables more accurately, thereby increasing [efficiency](../e/efficiency.md) and output.

### Catalysts and Enzymes

Improvements in catalytic materials and enzymatic processes are continually being developed to make refining processes more efficient and environmentally friendly.

### Digitalization and IoT

The integration of Internet of Things (IoT) devices enables real-time monitoring and predictive maintenance, reducing downtime and improving [efficiency](../e/efficiency.md).

## Major Players in the Oil Refinery Industry

### ExxonMobil

One of the world's largest publicly traded oil and gas companies, known for its extensive refining and petrochemical capabilities.

[ExxonMobil Refining](https://corporate.exxonmobil.com/what-we-do/fuels-and-lubricants/refining/overview)

### Royal Dutch Shell

A global group of energy and petrochemical companies, with a significant portion of its [business](../b/business.md) in refining operations.

[Shell Refining](https://www.shell.com/business-customers/trading-and-supply/refining-supply-and-distribution.html)

### BP (British Petroleum)

Another major player in the [industry](../i/industry.md) with a significant footprint in refining and petrochemicals.

[BP Refining](https://www.bp.com/en/global/corporate/who-we-are/our-businesses/refining.html)

### Chevron

Known for its integrated energy operations, Chevron also operates several major refineries around the world.

[Chevron Refining](https://www.chevron.com/operations/downstream)

## Future Trends

### Renewable and Green Technologies

The [industry](../i/industry.md) is increasingly focusing on developing renewable energy resources and integrating green technologies to reduce the environmental impact of refining processes.

### AI and Machine Learning

The use of [artificial intelligence](../a/artificial_intelligence_in_trading.md) and [machine learning algorithms](../m/machine_learning_algorithms_in_trading.md) can optimize various facets of refinery operations, from process control to predictive maintenance.

### Electric Vehicles (EVs) and Biofuels

The rise of electric vehicles and biofuels poses both challenges and opportunities for the refining [industry](../i/industry.md). While it may reduce [demand](../d/demand.md) for traditional [petroleum](../p/petroleum.md) products, it also opens avenues for producing alternative fuels.

## Conclusion

Oil refineries play a pivotal role in the global [supply chain](../s/supply_chain.md), transforming [crude oil](../c/crude_oil.md) into a multitude of products that power industries and daily life. Despite facing economic, environmental, and regulatory challenges, technological advancements and strategic adaptations continue to sustain their importance in the [energy sector](../e/energy_sector.md).
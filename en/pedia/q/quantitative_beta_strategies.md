...
Systematic beta capture involves constructing portfolios that capture the returns of market indices or specific asset classes systematically. This does not involve active stock picking or [market timing](../m/market_timing.md) but relies on rules-based approaches to track the performance of a benchmark index closely. Examples include index funds and exchange-traded funds (ETFs) that replicate the performance of indices like the S&P 500 or MSCI World.

#### Techniques Used

1. **Replication Strategies**: These involve direct replication by holding the same securities as in the benchmark index or synthetic replication using [derivatives](../d/derivatives.md) like futures and swaps to mimic the index returns.

2. **Optimization Models**: Techniques like [mean-variance optimization](../m/mean-variance_optimization.md) are used to construct portfolios that achieve the desired risk-return profile while closely tracking the benchmark.
...
[Risk parity](../r/risk_parity.md) strategies focus on allocating risk rather than capital to achieve better diversification and risk-adjusted returns. The idea is to equalize the risk contribution from different asset classes (e.g., equities, bonds, commodities) rather than allocating equal capital amounts. This often leads to higher allocations to lower volatility asset classes like bonds to balance the risk more evenly across the portfolio.
...
[Smart beta](../s/smart_beta.md) strategies blend passive and active management principles, using alternative weighting schemes or strategy indexes to outperform traditional market-cap-weighted indexes. These strategies aim to capture the [systematic risk](../s/systematic_risk.md) premia associated with different factors or investment styles without incurring the high costs and inefficiencies of active management.
...
- **BlackRock**: Offers a range of factor-based and [smart beta](../s/smart_beta.md) ETFs under its iShares brand.
...
- **AQR Capital Management**: Known for implementing advanced quantitative techniques in [factor investing](../f/factor_investing.md) and [risk parity](../r/risk_parity.md) strategies.
  - [AQR Capital Management](https://www.aqr.com/)
- **Dimensional Fund Advisors**: Applies academic research on [factor investing](../f/factor_investing.md) to create more efficient and effective investment solutions.

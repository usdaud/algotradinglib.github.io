# Average Price Order (APO)

An average price order is an execution instruction that aims to achieve an average price over a specified period rather than an immediate fill at a single price. It is commonly used for larger orders that need to reduce market impact.

## How It Works
The order is sliced into smaller trades and executed gradually. The target may be a time-weighted average or a volume-weighted average, depending on the algorithm and venue.

## Use Cases
- Institutional execution where minimizing market impact is important.
- Traders who want to avoid signaling their full order size.
- Portfolios that rebalance over a window rather than at a single point in time.

## Risks and Trade-Offs
APO execution can underperform if the market moves quickly against the order. Tight time windows increase urgency and cost, while longer windows increase exposure to market drift.

## Practical Notes
Different brokers and venues implement APO differently. Traders should verify how the order is benchmarked and what parameters can be controlled.
...
Consider a liquid instrument with stable spreads and average volatility. A rule based implementation can be tested on a multi year sample and then on an out of sample period. The goal is to verify that the behavior of Average Price Order (APO) is consistent across regimes and that the edge does not depend on a narrow set of conditions.

...
Average price is a simple measure of the typical price of an asset over a period or the average cost of executed trades. It is often used to evaluate execution quality or track a position's cost basis.

## Common Definitions
- Time average: sum of prices over a period divided by the number of observations.
- Fill average: total trade value divided by total quantity executed.

## Uses in Trading
Average price is used to compare execution against benchmarks, assess slippage, and estimate the average entry or exit price of a position. It can also support position management by clarifying profitability relative to the entry cost.

## Difference from VWAP
Average price does not weight by volume unless explicitly defined that way. VWAP is volume weighted and can differ significantly during periods of uneven trading activity.

## Limitations
A simple average can hide intraperiod volatility. When price distributions are skewed, the average may not reflect typical tradeable prices.
...
Consider a liquid instrument with stable spreads and average volatility. A rule based implementation can be tested on a multi year sample and then on an out of sample period. The goal is to verify that the behavior of Average Price is consistent across regimes and that the edge does not depend on a narrow set of conditions.

...
Advance-Decline Volume is a breadth measure that compares the volume flowing into advancing stocks versus declining stocks. It is designed to show whether buying pressure or selling pressure dominates across a market.

## Calculation
A common form is:
Advance-Decline Volume = Up Volume - Down Volume

Up Volume is total volume in stocks that closed higher, while Down Volume is total volume in stocks that closed lower. Some variants use a ratio rather than a difference.

## Interpretation
- Positive values suggest broad buying pressure.
- Negative values suggest broad selling pressure.
- Divergence between price and AD Volume can indicate weakening momentum.

## Use in Trading
Traders use this indicator to confirm breakouts and trends. A price breakout accompanied by strong positive AD Volume suggests healthier participation. Weak or negative AD Volume during a rally may signal a fragile move.

## Limitations
AD Volume can be skewed by a few high-volume names. It can also produce mixed signals during rotational markets, where capital shifts between sectors without a clear market-wide trend.

## Inputs and Parameters

Most implementations of Advance-Decline Volume use a lookback window and one or more smoothing steps. The lookback controls how quickly the indicator reacts; shorter values respond faster but produce more noise. Smoothing can be a simple moving average or another filter, and it should be consistent with the timeframe of the strategy.

When applying Advance-Decline Volume across different instruments, normalize inputs so the indicator remains comparable. For price based indicators, using median or typical price can reduce distortion from single prints. Volume based indicators require clean volume data and consistent session definitions.
...
Advance-Decline Volume tends to behave differently in trending and ranging markets. In strong trends it can stay above or below its baseline for extended periods, which makes mean reversion signals risky. In ranges it can oscillate frequently and generate whipsaws.

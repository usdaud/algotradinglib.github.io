...
### Definition
Weighted regression is a generalization of [linear regression](../l/linear_regression.md) where each data point is assigned a weight that indicates its influence on the regression curve. The goal is to minimize the weighted sum of squared residuals (the difference between the observed and predicted values).
...
### Python Libraries
- **Statsmodels:** A powerful library for statistical modeling which includes weighted regression capabilities through its `WLS` ([Weighted Least Squares](../w/weighted_least_squares.md)) class.

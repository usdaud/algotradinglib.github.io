...
- **Interactive Brokers**: Provides API access to build custom trading algorithms that can consider receipt and holding period of securities [Interactive Brokers](https://www.interactivebrokers.com/).
- **QuantConnect**: An open algorithmic trading platform that allows traders to design, test, and execute algorithms with custom tax law considerations [QuantConnect](https://www.quantconnect.com/).
- **TD Ameritrade's thinkorswim**: Offers scripting language ThinkScript, which can be used to build strategies that account for wash sales [TD Ameritrade](https://www.tdameritrade.com/).

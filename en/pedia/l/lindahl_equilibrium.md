...
The demand for a public good can be modeled using individual demand functions. Let \( D_i(P) \) represent the demand function of individual \( i \), where \( P \) denotes the price:
\[ D_i(P) = f_i(P) \]

### Total Demand and Supply

The aggregate demand is the sum of all individual demands:
\[ D_{total}(P) = \sum_{i=1}^{n} D_i(P) \]

The supply of the public good, \( S(G) \), is typically provided by the government or a centralized authority. The equilibrium condition requires:
\[ D_{total}(P) = S(G) \]

### Lindahl Prices

The Lindahl price for individual \( i \), denoted as \( P_i \), reflects their marginal benefit \( MB_i(G) \) from the public good \( G \):
\[ P_i = MB_i(G) \]

The equilibrium condition implies the sum of Lindahl prices equals the marginal cost of provision \( MC(G) \):
\[ \sum_{i=1}^{n} P_i = MC(G) \]

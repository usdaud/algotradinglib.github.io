...
A back ratio spread is an options strategy that combines a long option and a larger number of short options at a different strike. It is typically structured to receive a small credit or low debit and is sensitive to price movement and time decay.

## Structure
A common call back ratio spread uses:
- Buy 1 call at a lower strike
- Sell 2 calls at a higher strike

A put back ratio spread uses the equivalent structure with puts. The ratio can vary, but 1x2 is the most common.

## Payoff Profile
- Limited risk to the downside if constructed for a credit
- Maximum profit often occurs near the short strike at expiration
- Significant risk if price moves sharply through the short strike (unlimited for calls)

## Use Cases
Traders use back ratio spreads when they expect a modest move toward a target level and are willing to take risk if the move becomes too large. It can also be used as a volatility or time decay play if the position is opened for a credit.

## Practical Considerations
Implied volatility and skew matter because the strategy is net short options near the short strike. Risk should be stress tested for large moves. Early assignment risk exists if short options are in the money and carry little time value.

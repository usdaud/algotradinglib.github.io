...
A backspread is an options strategy that has more long options than short options. It is designed to profit from a large move in the underlying while limiting risk if the move does not occur.

## Structure
Backspreads can be constructed with calls or puts:
- Call backspread: short 1 call, long 2 calls at a higher strike
- Put backspread: short 1 put, long 2 puts at a lower strike

The position is often opened for a small debit or a credit depending on volatility and skew.

## Payoff Profile
- Limited risk around the short strike
- Potentially large profit if the underlying makes a strong move in the direction of the long options
- Time decay can work against the position if the move does not happen

## Use Cases
Backspreads are used when a trader expects a sharp directional move or a volatility expansion. They are common ahead of events such as earnings or economic releases.

## Risks
The biggest risk is a price close near the short strike at expiration, where the short option dominates. Gamma and vega exposure can be significant, so position sizing and scenario analysis are important.

...
A Bollinger squeeze occurs when Bollinger Bands contract to an unusually narrow range. This indicates low volatility and often precedes a significant price move.

## Identification
The squeeze is commonly detected using Bollinger Bandwidth reaching a low percentile relative to its historical range.

## Trading Approach
Traders watch for a breakout from the squeeze with increasing volume or momentum. Direction is not implied by the squeeze alone, so confirmation is important.

## Risks
False breakouts can occur, especially in choppy markets. Risk management with stops and position sizing is essential.

## Inputs and Parameters

Most implementations of Bollinger Squeeze use a lookback window and one or more smoothing steps. The lookback controls how quickly the indicator reacts; shorter values respond faster but produce more noise. Smoothing can be a simple moving average or another filter, and it should be consistent with the timeframe of the strategy.

When applying Bollinger Squeeze across different instruments, normalize inputs so the indicator remains comparable. For price based indicators, using median or typical price can reduce distortion from single prints. Volume based indicators require clean volume data and consistent session definitions.
...
Bollinger Squeeze tends to behave differently in trending and ranging markets. In strong trends it can stay above or below its baseline for extended periods, which makes mean reversion signals risky. In ranges it can oscillate frequently and generate whipsaws.
...
A common approach is to use Bollinger Squeeze as a confirmation layer rather than a standalone trigger. For example, a breakout setup can be filtered by requiring the indicator to be positive and rising. In mean reversion systems, the indicator can define the strength of the counter move before entry.

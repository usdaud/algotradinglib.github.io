...
A busted trade is a transaction that is canceled by an exchange or broker due to a clear error, abnormal price, or technical issue. Rules vary by venue and define when a trade can be broken.

## Common Causes
- Erroneous order entry or fat finger mistakes
- Extreme price dislocations or system malfunctions
- Trades executed outside defined price bands

## Process
Exchanges typically have a review window during which participants can request a cancellation. If approved, the trade is removed from the official record.

## Trading Impact
Busted trades can create uncertainty in positions and PnL. Traders should monitor for potential breaks when abnormal prints occur.

## Limitations
Not all bad trades are eligible for cancellation. The criteria are strict and time sensitive, so rapid detection is important.
...
Consider a liquid instrument with stable spreads and average volatility. A rule based implementation can be tested on a multi year sample and then on an out of sample period. The goal is to verify that the behavior of Busted Trade is consistent across regimes and that the edge does not depend on a narrow set of conditions.

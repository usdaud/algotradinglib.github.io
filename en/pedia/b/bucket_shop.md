...
A bucket shop is a historical term for a brokerage operation that took the opposite side of customer trades without actually executing them in the market. These operations were often illegal and relied on customer losses for profit.

## How It Worked
Customers believed they were trading real securities, but the bucket shop simply recorded the bets internally. Winning customers might be denied or delayed, and losing customers funded the business.

## Modern Relevance
Most jurisdictions now regulate such practices heavily. The term is still used to describe unregulated or fraudulent trading operations.

## Risks
Bucket shops expose traders to counterparty fraud, price manipulation, and lack of legal recourse. Due diligence on broker regulation and execution practices is essential.
...
Consider a liquid instrument with stable spreads and average volatility. A rule based implementation can be tested on a multi year sample and then on an out of sample period. The goal is to verify that the behavior of Bucket Shop is consistent across regimes and that the edge does not depend on a narrow set of conditions.

...
A buy on open order is an instruction to purchase a security at the opening price. The order is executed during the opening auction or the first trade of the session.

## Use Cases
- Strategies that react to overnight news
- Rebalancing at the start of the trading day
- Capturing early session momentum

## Execution Considerations
Opening auctions can be volatile and sensitive to imbalances. The final opening price may differ significantly from pre-market indications.

## Risks
Gaps between the prior close and the open can lead to unfavorable fills. Traders should use price limits if execution certainty is less important than price control.

...
Bid-ask bounce is a microstructure effect where transaction prices alternate between the bid and the ask even when the underlying value is unchanged. This can create artificial volatility and misleading short-term signals.

## Why It Happens
Trades occur at the bid when sell orders are aggressive and at the ask when buy orders are aggressive. The alternating sequence can appear as price reversals in tick data.

## Impact on Analysis
Bid-ask bounce can distort returns, trigger false momentum signals, and inflate volatility measures on very short time frames.

## Mitigation
Analysts often use mid prices, quote midpoints, or sampling at lower frequency to reduce the effect. Filtering techniques can also help stabilize short-term indicators.
